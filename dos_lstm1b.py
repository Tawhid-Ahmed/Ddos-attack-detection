...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb  5 18:54:22 2023
...
# def LstmModel(model_name,input_shape,kernel_col, kernels=64,kernel_rows=3,learning_rate=0.01,regularization=None,dropout=None):
#     K.clear_session()

#     Lmodel = Sequential(name=model_name)
#     regularizer = regularization

#     Lmodel.add(tf.keras.layers.LSTM(input_shape, kernel_regularizer=regularizer))
#     if dropout != None and type(dropout) == float:
#         model.add(Dropout(dropout))
#     Lmodel.add(Activation('relu'))
#     Lmodel.add(tf.keras.layers.LSTM(input_shape/2, kernel_regularizer=regularizer))
#     Lmodel.add(tf.keras.layers.PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None))
#     Lmodel.add(tf.keras.layers.LSTM(input_shape/4, kernel_regularizer=regularizer))
#     Lmodel.add(tf.keras.layers.PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None))
#     Lmodel.add(Flatten())
#     Lmodel.add(Dense(1, activation='sigmoid', name='fc1'))

#     print(Lmodel.summary())
#     compileModel(Lmodel, learning_rate)
#     return Lmodel
def LSTM1bModel(model_name,input_shape,kernel_col,learning_rate=0.01,kernels=64,kernel_rows=3,regularization=None,dropout=None):
    K.clear_session()
    ...
    #model.add(tf.keras.layers.Reshape((input_shape[0],input_shape[1]*input_shape[2],input_shape[3]), input_shape=input_shape))
    model.add(tf.keras.layers.Reshape((input_shape[0] * input_shape[1], input_shape[2]), input_shape=input_shape))
    # model.add(tf.keras.layers.LSTM(128, return_sequences=True, kernel_regularizer=regularizer, name='lstm0'))
    # model.add(Activation('tanh'))
    model.add( (tf.keras.layers.LSTM(4, return_sequences=True, kernel_regularizer=regularizer, name='lstm0')))
    if dropout != None and type(dropout) == float:
        model.add(Dropout(dropout))
    model.add(tf.keras.layers.BatchNormalization())    
    model.add(Activation('relu'))
    # model.add(tf.keras.layers.LSTM(32, return_sequences=True, kernel_regularizer=regularizer, name='lstm2'))
    # model.add(Activation('relu'))
    #model.add( (tf.keras.layers.LSTM(16, return_sequences=False,name='lstm1')))
    model.add(Flatten())
    ...
    return model
# def LSTMModel(model_name,input_shape,kernel_col,learning_rate=0.01,kernels=64,kernel_rows=3,regularization=None,dropout=None):
#     model = tf.keras.Sequential(name=model_name)
#     regularizer = regularization
#     model.add(tf.keras.layers.Reshape((input_shape[0] * input_shape[1], input_shape[2]), input_shape=input_shape))
#     model.add(tf.keras.layers.LSTM(64, return_sequences=True, kernel_regularizer=regularizer, name='lstm0'))
#     model.add(tf.keras.layers.BatchNormalization())
#     model.add(tf.keras.layers.Activation('tanh'))
#     model.add(tf.keras.layers.LSTM(32, return_sequences=True, kernel_regularizer=regularizer, name='lstm1'))
#     if dropout is not None and isinstance(dropout, float):
#         model.add(tf.keras.layers.Dropout(dropout))
#     model.add(tf.keras.layers.BatchNormalization())
#     model.add(tf.keras.layers.Activation('tanh'))
#     model.add(tf.keras.layers.LSTM(16, return_sequences=False, name='lstm2'))
#     model.add(tf.keras.layers.BatchNormalization())
#     model.add(tf.keras.layers.Activation('relu'))
#     model.add(tf.keras.layers.Dense(1, activation='sigmoid', name='fc1'))
...
def main(argv):
    ...
    if args.train is not None:
        ...
        for full_path in subfolders:
            ...
            model_name = dataset_name + "-dos_lstm1b"
            keras_classifier = KerasClassifier(build_fn=LSTM1bModel,model_name=model_name, input_shape=X_train.shape[1:] ,kernel_col=X_train.shape[2])#,kernel_col=X_train.shape[2], X_train.shape[1:]
            ...
            print("training set shape",X_train.shape[1:])
